        ...
        protected override void OnUpdate()
        {
            ...
            if (mouse.leftButton.wasPressedThisFrame)
            {
                ...
                if (Camera.main != null)
                {
                    ...
                    {
                        var arr = GetEntityQuery(ComponentType.ReadOnly(typeof(PieceComponent)))
            ...
            if (_sp_touching && !_sp_holding)
            {
                ...
                {
                    _sp_holding = true;
                    var holdEcb = ((BeginSimulationEntityCommandBufferSystem)World.GetOrCreateSystemManaged(typeof(BeginSimulationEntityCommandBufferSystem))).CreateCommandBuffer();
            ...
            if (_sp_holding)
            {
                ...
                if (delta.magnitude >= DragThreshold)
                {
                    ...
                    {
                        var clearEcb =
                            ((BeginSimulationEntityCommandBufferSystem)World.GetOrCreateSystemManaged(
                                typeof(BeginSimulationEntityCommandBufferSystem))).CreateCommandBuffer();
                        var clearEntity = clearEcb.CreateEntity();
                        clearEcb.AddComponent(clearEntity,
                            new ColorClearRequest { TargetColor = foundColor, IsFromSpecialPiece = true });
                    }
                }
            }
